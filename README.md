...
**Note**: this macro is dependent on a files named `{$input_imagename}_cell.ROI` being found in the output directory. This macro, as above, will also skip all images which have already been analysed (see lines 35-47).
...
\***Note**: data from different experiments/repeats were pooled together in this step, so the prefix below was added to all file names.  
`JEP{expt_num}-{pHmS_prot}-{pHl_prot}_` (If I remember correctly)
...
- `exo_script.R`
  ...
  - `threshold2` - a threshold for peak detection. Input as a percentage of the peak value
- `exo_functions.R`
